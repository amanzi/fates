module ATSFatesParamInterfaceMod
  ! NOTE(bja, 2017-01) this code can not go into the main ats-fates
  ...
  ! NOTE(bja, 2017-01) these methods can NOT be part of the atsi-fates
  ...
  integer :: iulog = 10
  integer :: masterproc
  character(len=256) :: fates_paramfile
...
contains
 ...
 subroutine FatesReadParameters() BIND(C)
   ...
   ! character, dimension(*) :: paramfile
   ! character, dimension(*) :: fates_paramfile
   character(10000), pointer :: lcl_str
   integer :: str_len, ierr
   character(len=256) :: fates_paramfile_const = "../parameter_files/fates_params_c052019.nc"
   character(len=256) :: paramfile_const = "../parameter_files/clm_params_c180301.nc"
   character(len=32)  :: subname = 'FatesReadParameters'
   class(fates_parameters_type), allocatable :: fates_params   
   logical :: is_host_file
   
   if (masterproc) then
      write(iulog, *) 'atsfates_parameterinterfaceMod.F90::'//trim(subname)//' :: ATS reading ED/FATES '//' parameters '
   end if

   call ncd_pio_init()

   write(*,*) paramfile, fates_paramfile
   ...
   is_host_file = .false.
   call ParametersFromNetCDF(masterproc, fates_paramfile, is_host_file, fates_params)

   is_host_file = .true.
   call ParametersFromNetCDF(masterproc, paramfile, is_host_file, fates_params)
 ...
 !   use ats_varctl, only : use_fates, paramfile, fates_paramfile
   ...
   if (masterproc) then
      write(fates_log(), *) 'atsfates_interfaceMod.F90::'//trim(subname)//' :: ATS reading ED/FATES '//' PFTs '
   end if
...
! -----------------------------------------------------------------------
 ...
 !-----------------------------------------------------------------------
 subroutine ParametersFromNetCDF(masterproc, filename, is_host_file, fates_params)
   ...
   integer, intent(in) :: masterproc  
   logical, intent(in) :: is_host_file
   class(fates_parameters_type), intent(inout) :: fates_params

   character(len=32)  :: subname = 'atsfates_interface::ReadParameters'
   character(len=256) :: locfn ! local file name
   character(len=256) :: fates_paramfile_const = "../parameter_files/fates_params_c052019.nc"
   type(file_desc_t)  :: ncid  ! pio netCDF file id
   ...
   call getfil (iulog, masterproc, filename, locfn, 0)   
...
end module ATSFatesParamInterfaceMod
