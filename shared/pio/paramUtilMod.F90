...
!         call ncd_inqvdims(ncid, num_dims, var_desc)
         ...
         !    call endrun(msg)
         ! end if
         ! do d = 1, expected_numDims
         !    received_dimName = ''
         !    !call ncd_inqvdname(ncid, varname=trim(varName), dimnum=d, dname=received_dimName, err_code=error_num)
         !    call ncd_inqdid(ncid, name=expected_dimNames(d), dimid=dim_id, dimexist=dim_exist)
         !    call ncd_inqdname(ncid, dimid=d, dname=received_dimName)
